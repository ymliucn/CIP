...
class Dataset:
    def __init__(self, file_path):
        ...
        print(f'数据集{self.file_name}：句子数：{self.sentence_num},  总词数：{self.total_word_num}'
    ...
    def load(self, file_path):
        ...
        for line in corpus:
            if len(line) >= 5:
                ...
                sentence = []
        corpus.close()
        self.id2tag = sorted(list(tag_set))
        self.tag2id = {tag: index for index, tag in enumerate(self.id2tag)}
